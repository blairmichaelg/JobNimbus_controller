...
async def test_parse_statement_of_loss(tmp_path):
    ...
    to return a simulated JSON structure without hitting the API.
    """
    # Create a dummy PDF file (pdfplumber requires a real file, but we will mock pdfplumber too if needed, 
    # or just mock _extract_raw_tables to avoid needing a valid PDF).
    dummy_pdf = tmp_path / "dummy.pdf"
    dummy_pdf.touch()

    # The expected JSON response from Gemini
    ...
    with patch("app.services.document_parser._extract_raw_tables", return_value=("Dummy Text", [{"page": 1, "cells": ["RFG", "300"]}])), \
         patch("app.services.ai_service.get_ai_client") as mock_get_client:
        
        mock_ai_client = MagicMock()
        # extract_sol_structured_data is awaited in _gemini_extract
        async def mock_extract(*args, **kwargs):
            return mock_gemini_json
            
        mock_ai_client.extract_sol_structured_data = mock_extract
